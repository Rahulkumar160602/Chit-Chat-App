...
export const createNewChat = TryCatch(async (req: AuthenticatedRequest, res) => {
  ...
  // Safely get userId from either id or _id
  ...
  const chatWithUserData = await Promise.all(
    chats.map(async (chat) => {
      ...
      const unseenCount = await Messages.countDocuments({
        ...
        sender: {
          $ne: userId, // Count only unseen messages from the other user
        }, 
        // Count only unseen messages from the other user
        seen: false,
      });
...
export const getMessagesByChat= TryCatch(async(req: AuthenticatedRequest, res) => {
  ...
  const messagesToMarkSeen = await Messages.find({
    chatId: chatId,
    sender: {
      $ne: userId, // Only mark messages from the other user as seen
    },
    seen: false,
  });
