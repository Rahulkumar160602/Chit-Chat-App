...
const VerifyOtp = () => {
  ...
  const handleSubmit = async (e:React.FocusEvent<HTMLFormElement>) => {
    ...
    if(otpString.length !==6){
      setError("Please Enter all 6 didgits");
      return;
    }
    setError("")
    setLoading(true)
    try {
      const {data}=await axios.post(`${user_services}/api/v1/verify`,{
        email,
        otp:otpString,
      });
  ...
  const handleResendOtp =async()=>{
    ...
    setError("")
    try{
      const {data}= await axios.post(`${user_services}/api/v1/login`,{
        email,
  ...
  return (
    <div className="min-h-screen bg-gray-900 flex items-center justify-center p-4">
      <div className="max-w-md w-full">
        <div className="bg-gray-800 border border-gray-700 rounded-lg p-8 flex flex-col items-center">
              ...
              <div className="flex justify-center space-x-3">
                {otp.map((digit, index) => (
                  <input
                    ...
                    className="w-12 h-12 text-center text-xl font-bold border-2 border-gray-600 rounded-lg bg-gray-700 text-white focus:border-blue-500 outline-none"
                  />
                ))}
              </div>
            </div>
          ...
          <div className="mt-6 text-center">
            <p className="text-gray-400 text-sm mb-4">Din't recive the code</p>
            {
