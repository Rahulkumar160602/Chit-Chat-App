...
import { chat_services, useAppData, User } from '@/context/AppContext'
...
const ChatApp = () => {
  ...
  async function fetchChat(){
    ...
    try {
      const {data} =await axios.get(`${chat_services}/api/v1/message/${selectedUser}`,{
        headers:{
  ...
  async function createChat(u:User){
    try{
      const token = Cookies.get("token");
      const {data} =await axios.post(`${chat_services}/api/v1/chat/new`,{
  ...
  const handleMessageSend=async(e:any, imageFile? :File | null)=>{
    ...
    try {
      ...
      const {data} =await axios.post(`${chat_services}/api/v1/message`,formData,{
        headers:{
          Authorization: `Bearer ${token}`,
...
useEffect(()=>{
  ...
  socket?.on("userStopTyping",(data)=>{
    console.log("received user stop typing", data);
    if(data.chatId === selectedUser && data.userId !== loggedInUser?._id){
      setIsTyping(false);
    }
  });
  ...
  return (
    <div className='min-h-screen flex  bg-gray-900 text-white relative overflow-hidden'>
      ...
      showAllUsers={showAllUser} setShowAllUsers={setShowAllUser}
      users={users} loggedInUser={loggedInUser} chats={chats} 
      ...
      isTyping={isTyping} onlineUsers={onlineUsers}/>
      <ChatMessages 
